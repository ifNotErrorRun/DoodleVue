<script setup>
  ...
  const removeItem = address => {
    seearchData.value = seaerchData.value.filter(item => item.address !== address)
  }
...
<template>
  <main class="weather-city">
    ...
    <section class="weather__search">
      <input v-model="city" type="text" class="weather__searchBar" placeholder="검색할 지역을 입력해 주세요." @keyup.enter="searchWeather" />
    </section>
    <!-- 검색 데이터가 있으면 -->
    <section v-for="data in searchData" :key="data.address" class="weather__city">
      ...
      <div class="weather__cityRight">
        <img :src="getImage(data.icon)" :alt="`${data.address} ${data.temp}도`" class="weather__cityImg" />
      </div>
